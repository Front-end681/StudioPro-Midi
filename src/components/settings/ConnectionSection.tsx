...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, icon, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`bg-[#1a1a1a] border border-[#2e2e2e] rounded-2xl w-full max-w-[480px] ${isMobile ? 'max-h-[95vh]' : 'max-h-[90vh]'} overflow-y-auto no-scrollbar shadow-2xl relative animate-in zoom-in-95 duration-200`}>
        <button 
          onClick={onClose}
...
const CodeBlock: React.FC<{ command: string }> = ({ command }) => {
  ...
  return (
    <div className="bg-[#0f0f0f] border border-[#2e2e2e] rounded-lg p-3 pr-12 font-mono text-xs md:text-sm text-[#1D9E75] relative group mb-3 break-all">
      {command}
      <button 
        onClick={handleCopy}
...
`;
  ...
  return (
    <section>
      <h2 className="text-[10px] uppercase tracking-[0.2em] text-[#888] font-bold mb-4">Connection</h2>
      <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
        ...
        <div className="bg-[#1a1a1a] rounded-xl border border-[#2e2e2e] p-5 flex flex-col justify-between group relative">
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              <div className="text-sm font-bold">USB MIDI</div>
              <div className="text-[11px] text-[#666]">Connect via USB cable</div>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <div className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider ${usbDevice ? 'bg-[#1D9E75]/10 text-[#1D9E75] border border-[#1D9E75]/20' : 'bg-[#242424] text-[#666] border border-[#2e2e2e]'}`}>
        ...
        <div className="bg-[#1a1a1a] rounded-xl border border-[#2e2e2e] p-5 flex flex-col justify-between group relative">
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              <div className="text-sm font-bold">WiFi MIDI</div>
              <div className="text-[11px] text-[#666]">Connect over your local network</div>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <div className="flex flex-col gap-1">
              <div className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider inline-flex items-center gap-1.5 ${
              ...
              {wifiStatus === 'connected' && (
                <div className="text-[10px] font-mono text-[#666] ml-1">{wifiIP}:{wifiPort}</div>
              )}
            </div>
          </div>
        </div>
      ...
      >
        <div className="space-y-6">
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Requirements</h3>
          ...
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Steps</h3>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex gap-4">
                  <div className="w-6 h-6 rounded-full bg-[#242424] border border-[#2e2e2e] flex items-center justify-center text-[10px] font-bold text-[#1D9E75] shrink-0">
                    {i + 1}
                  </div>
                  <p className="text-sm text-[#f0f0f0] leading-relaxed">{step}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Common Devices</h3>
            <ul className="space-y-2 text-sm text-[#888]">
          ...
          <div className="p-4 bg-[#E24B4A]/5 border border-[#E24B4A]/20 rounded-xl">
            <div className="text-[10px] font-bold text-[#E24B4A] uppercase tracking-widest mb-1 flex items-center gap-2">
              ⚠️ Not working?
            </div>
            <p className="text-xs text-[#888]">Make sure you're using Chrome or Edge. Firefox does not support WebUSB.</p>
          </div>

          {usbError && (
            <div className="p-3 bg-[#E24B4A]/10 border border-[#E24B4A]/20 rounded-lg text-xs text-[#E24B4A]">
              {usbError}
            </div>
          )}
      ...
      >
        <div className="space-y-6">
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">How it works</h3>
          ...
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Step 1 — Install Node.js</h3>
            <div className="flex items-center justify-between bg-[#242424] p-3 rounded-xl border border-[#2e2e2e]">
              <div className="text-sm text-[#f0f0f0]">Download from: nodejs.org</div>
              <a href="https://nodejs.org" target="_blank" rel="noopener noreferrer" className="text-[#1D9E75] hover:underline">
                <ExternalLink size={16} />
              </a>
            </div>
            <p className="text-[10px] text-[#666] mt-2">(one-time setup)</p>
          </section>

          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Step 2 — Get server.js</h3>
            <button 
          ...
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Step 3 — Run the server</h3>
            <p className="text-sm text-[#888] mb-3">Open Terminal and run:</p>
            <CodeBlock command="npm install ws midi" />
            <CodeBlock command="node server.js" />
          </section>

          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Step 4 — Find your computer's IP</h3>
          ...
          <section>
            <h3 className="text-[11px] font-bold text-[#666] uppercase tracking-widest mb-3">Step 5 — Connect</h3>
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="col-span-2 space-y-2">
              <label className="text-[10px] font-bold text-[#666] uppercase tracking-widest ml-1">IP Address</label>
              <input 
                type="text" value={wifiIP} onChange={(e) => updateSetting('wifiIP', e.target.value)}
                placeholder="192.168.1.100"
                className="w-full bg-[#0f0f0f] border border-[#2e2e2e] rounded-xl px-4 py-2.5 text-sm text-[#f0f0f0] focus:border-[#1D9E75] outline-none"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-[#666] uppercase tracking-widest ml-1">Port</label>
              <input 
          ...
          <div className="p-4 bg-[#EF9F27]/5 border border-[#EF9F27]/20 rounded-xl">
            <div className="text-[10px] font-bold text-[#EF9F27] uppercase tracking-widest mb-1 flex items-center gap-2">
              ⚠️ Network Tip
            </div>
            <p className="text-xs text-[#888]">Both devices must be on the same WiFi network for the connection to work.</p>
          </div>
        </div>
      </Modal>
    </section>
